/**  
 * Copyright ©MoveBroad Ltd.,co. All rights reserved.
 * @Title: server_data_process
 * @Prject: WebserviceData
 * @Description: Traffic flow data reception with gateway device
 * @author: Rui SUN  
 * @date: 24/08/2017
 * @version: V1.0  
 */
...
var data;
var length_will_rcv = 0;//the length ought to be received as a package
var length_is_rcv = 0;//the length of a frame 
var counter_suc = 0;
var counter_fai = 0;
...
server.on('connection', (socket) => {
    ...
    num_socket++;
    console.log('New socket connected: ' + num_socket + '\'s socket ever');
    ...
    socket.on('data', (data_rcv) => {
        var is_header = true;
 //Identify the header location by CrossID: 8 bytes number
        for (var i = 0; i <= 7; i++) {
...
var handle_data = function (data, socket) {
    //Double verification of length, could be deleted
    ...
    //verifier if the packet is for asychronous time and for storage
    if ((check_code == check_correct) && num_correct &&data[8] == 128) {
        Res_time(data, socket);
    }
    if ((check_code == check_correct) && num_correct &&data[8] == 129) {
...
//Respond success reception after storage
